...
function undoMove() {
  if (game) { // check if the game is still in progress
    if (round > 0) { // check if there is a previous move to undo
      round--;
      var lastMove = game.moves.pop(); // get the last move from the moves array
      board[lastMove] = 0; // reset the board at the last move position
      var lastSign = document.getElementById(signs[lastMove].id); // get the last sign element
      lastSign.classList.remove(humVal, comVal); // remove the class of the last player from the sign element
      lastSign.classList.add('empty'); // add the empty class to the sign element
      // switch turns back to the previous player
      player1 = !player1;
      player2 = !player2;
      // update the message to show the current player
      setMessage(player1 ? `${palyer1}'s turn` : `${player2}'s turn`);
    }
  }
}
